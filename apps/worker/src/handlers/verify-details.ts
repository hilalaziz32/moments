...
export const verifyDetailsSend: TaskHandler = {
  ...
  async handle(ctx) {
    const { data: event } = await ctx.db
      .from("moment_events")
      .select("id, org_id, employee_id, occurs_on, employees(full_name, preferred_name, work_email, personal_email, whatsapp_e164)")
    ...
    const emp = event.employees as unknown as {
      ...
      work_email: string | null; personal_email: string | null; whatsapp_e164: string | null;
    } | null;

    const recipient = emp?.work_email ?? emp?.personal_email;
    if (!recipient) {
      // No way to reach them at all. Retrying will not conjure an address.
      throw new PermanentTaskError("employee has no email address on file");
    }
    ...
    return { status: "done", result: { sentTo: "employee", channel: "email" } };
  },
};
